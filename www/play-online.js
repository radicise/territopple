...
function displayRooms(text) {
    ...
    /**@type {{ident:string,capacity:number,playing:number,spectating:number,dstr:string,can_spectate:boolean,phase:"wait"|"play"|"over",res:boolean,pw:boolean}[]} */
    ...
    for (const game of games) {
        ...
        row.scope = "row";
                ...
                link.href = `http://${document.location.host}/territopple?t=0&g=${game.ident}`+(game.res?"&res=1":"")+(game.pw?"&pw=1":"");
