...
function handle(name, sock, args, state, __tag) {
    ...
    }, emit:(name, data) => {data["#gameid"]=state.game.ident;emit(genTag, name, data);}, on:(name, cb) => {on(genTag, name, (data, tag) => {if (data["#gameid"]===state.game.ident&&tag!==genTag)cb(data, tag);});}}, args||{}, state);
    messageL = m.messageL;
    closeL = m.closeL;
    errorL = m.errorL;
    return genTag;
