...
const handler = (sock, globals, {change, emit, onall, on}, args, state) => {
    ...
    if (!(rgameId in globals.state.games)) {
        change("error", {code:1,data:"game id does not exist",redirect:"/",store:"game id does not exist"});
        return;
    }
    ...
    if (args.asSpectator??false) {
        if (!game.state.observable) {
            change("error", {data:"spectators aren't allowed in this room",redirect:"/",store:"spectators aren't allowed in this room"});
            return;
        }
        state.game = game;
    ...
    if (game.stats.playing + game.stats.reservedSlots >= game.stats.maxPlayers) {
        change("error", {data:"room is full",redirect:"/",store:"room is full"});
        return;
    }
    if (game.state.state !== 0) {
        change("error", {code:2,data:"game in progress",redirect:"/",store:"game in progress"});
        return;
    }
    state.game = game;
